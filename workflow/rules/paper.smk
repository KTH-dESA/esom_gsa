...
rule setup_paper_data:
    input:
        ...
        config = expand("../config/model_{model}/config.csv", model = MODELS),
        notebooks = expand("notebooks/model_{model_num}.ipynb", model_num = [1,2,3]),
...
rule copy_paper_model_data:
    ...
    run:
        ...
        dir_path = Path("..", "config", model_wildcards.model, "data")
        if not dir_path.exists():
...
rule copy_paper_model:
    ...
    run: 
        dir_path = Path("..", "resources", model_wildcards.model)
        if not dir_path.exists():
            dir_path.mkdir()
...
rule copy_paper_gsa_data:
    ...
    output:
        ...
        config = "../config/model_{model}/config.csv",
    run:
        import shutil 
        from pathlib import Path

        dir_path = Path("config", model_wildcards.model)
        if not dir_path.exists():
            dir_path.mkdir()
...
rule copy_paper_notebooks:
    ...
    output:
        notebook = "notebooks/model_{model_num}.ipynb"
    wildcard_constraints:
        model_num = "[0-9]"
    run:
        import shutil 
